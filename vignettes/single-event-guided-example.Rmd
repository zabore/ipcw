...
`get_ipcw_boot()`. Then, create the plot with the `plot_ipcw_km_boot_ci()` 
function. Note that depending on the size of the data and the number of 
bootstrap samples, the bootstrapping step can take some time to run. 
The bootstrapped data for this guided example is included in the package,
so we do not execute the code below.
