...
`get_ipcw_boot()`. Note that depending on the size of the data and the number of 
...
Then, create the plot with the `plot_ipcw_km_boot_ci()` function. This too can 
...
ggsave("vignettes/ipcw_km_boot_ci.png", width = 6, height = 4, dpi = 150)
```

```{r boot-plot-show, echo = FALSE}
...
Use the convenience wrapper `get_ipcw_cox_fit()` to obtain a tibble of results from the weighted Cox regression model.
...
Use the function `get_boot_pci()` on a dataframe of the log(HR)s to obtain the bootstrap-based 95% confidence interval.
